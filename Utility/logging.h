...
#define FATAL(x) \
    spdlog::get("log")->error("[{}]{} System exit.", unit_name, x), abort

#define ERROR_P(x) \
    spdlog::get("log")->error("[{}]{}.{}.", unit_name, x, strerror_tl(errno))

#define FATAL_P(x)                                                    \
    spdlog::get("log")->error("[{}]{} System exit.{}.", unit_name, x, \
                              strerror_tl(errno)),                    \
        abort()
...
class LOG {
   public:
    ...
    ~LOG() { spdlog::get("log")->flush(); }

   private:
    //配置日志文件、写入模式
    LOG() {
        //日志实例名、日志文件最大大小、滚动文件数量（日志太多的时候，当前文件重命名_1,_2,_3.再写新的文件）
        auto logger = spdlog::rotating_logger_mt("log", "log/log.txt",
                                                 5 * 1024 * 1024, 10);
